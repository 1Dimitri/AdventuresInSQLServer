...
-- fn_getdetaileddatabaserolepermissions
-- 
...
CREATE FUNCTION [dbo].[fn_getdetaileddatabaserolepermissions]()
RETURNS TABLE 
AS
RETURN
...
sch.name +'.'+objs.name as objectname,
...
join sys.objects as objs on sp.major_id = objs.object_id
join sys.schemas as sch on sch.schema_id = objs.schema_id
where sid1.type='R'
GO
